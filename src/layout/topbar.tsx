...
const Topbar: React.FC<IProps> = ({ location }) => {
  ...
  return (
    <nav className="border-b bg-white fixed top-0 w-screen z-10 shadow-sm">
      <div className="container my-4 flex justify-between items-center">
        <h3 className="text-xl font-bold">
          <span className="text-orange-500">Orange</span>Weather
        </h3>
        <div className="relative hidden md:block">
          <div className="relative hidden md:block">
            <Input
              placeholder="Search Location"
              className="w-96 pr-9 text-gray-500"
              onChangeCapture={handleInputChange}
            />
            <Search className="w-4 h-4 absolute top-1/2 right-4 -translate-y-1/2 text-gray-400" />
            {inputValue && (
              <div className="fixed bg-white w-96 p-4 z-10 border-b border-r border-l shadow-md">
                {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                {data?.map((item: any) => (
                  <span
                    key={item?.lat}
                ...
                {loading && (
                  <span className="block p-2 cursor-pointer hover:text-orange-500">
                    Loading...
                  </span>
                )}
                {error && (
              ...
              <DialogHeader>
                ...
                <DialogDescription>
                  <div className="relative">
                    <Input
                      placeholder="Search Location"
                      className="w-80 pr-9 text-gray-500 mx-auto"
                      onChangeCapture={handleInputChange}
                    />
                    {inputValue && (
                      <div className="fixed bg-white w-96 p-4 z-10 border-b border-r border-l shadow-md">
